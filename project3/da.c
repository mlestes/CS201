...
void insertDA(DA *item, void *value){
    ...
    if(item->capacity == item->size){
        void **temp = newArray(2 * item->capacity);
        for(int i = 0; i < item->size; i++){
            temp[i] = item->array[i];
            }
        item->array = temp;
        item->capacity = 2 * item->capacity;
        }

    item->array[item->size] = value;
    item->size++;
...
void *removeDA(DA *item){
    ...
    assert(item->size > 0);
    item->size--;
    void *t = item->array[item->size];
    item->array[item->size] = NULL;
    if(.25 > (double)item->size / item->capacity && item->capacity > 1){
        void **temp = newArray(item->capacity / 2);
        for(int i = 0; i < item->size; i++){
            temp[i] = item->array[i];
            }
        item->array = temp;
